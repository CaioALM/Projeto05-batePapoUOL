...
function startEventListeners() {
  document
    ...
    .addEventListener("keyup", checkUsernameInput);
...
function requestUsername(username) {
  ...
  const usernameRequest = axios.post(
    "https://mock-api.driven.com.br/api/v6/uol/participants",
    nameObject
  );
  usernameRequest.then(function () {
    successfulLogin();
  });
...
function loadingScreenToggle() {
  initialElements = document.querySelectorAll(
    "#login-screen > *:not(.logo, .escondido)"
  );
  loadingElements = document.querySelectorAll(
    "#login-screen > .escondido:not(.logo)"
  );
...
function requestMessagesLoop() {
  requestMessages();
  setInterval(requestMessages, 4000);
}
...
function renderMessages(response) {
  ...
  JSON.stringify(messagesArray) !== JSON.stringify(lastMessages);
    ...
    for (let i = 0; i < messagesArray.length; i++) {
      switch (messagesArray[i].type) {
        ...
        case "private_message":
          if (
            messagesArray[i].to === "Todos" ||
            messagesArray[i].to === nameObject.name ||
            messagesArray[i].from === nameObject.name
          ) {
            message = `
...
function requestParticipants() {
  const participantsRequest = axios.get(
    "https://mock-api.driven.com.br/api/v6/uol/participants"
  );
...
function renderParticipants(response) {
  ...
  participantOnline(participantsArray);
  let participant;
  let classes = "check escondido";
...
function participantOnline(participantsArray) {
  const stillHere = participantsArray.find(
    (participants) => participants.name === messageToSend.to
  );
...
function selectPrivacy(privacy) {
  ...
  if (privacy.innerText === "Reservadamente") {
    ...
    privacyChecks[0].classList.add("escondido");
    privacyChecks[1].classList.remove("escondido");
  } else {
    messageToSend.type = "message";
    privacyChecks[1].classList.add("escondido");
    privacyChecks[0].classList.remove("escondido");
  }
  updateDestination();
}
function showSidebar(bol) {
  ...
  if (bol) {
    rightDiv.classList.remove("escondido");
    setTimeout(function () {
    ...
    setTimeout(function () {
      rightDiv.classList.add("escondido");
    }, 300);
  }
}
...
function updateCheck(participant) {
  const checkedParticipant = participant.parentNode.querySelector(
    "li span.check:not(.escondido)"
  );
  checkedParticipant.classList.add("escondido");
  participant.querySelector("span.check").classList.remove("escondido");
}
